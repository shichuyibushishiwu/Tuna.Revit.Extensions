...
namespace Tuna.Revit.Extension
{
    ...
    public static class ElementExtension
    {
        ...
        /// <returns></returns>
        public static SelectionResult<FilteredElementCollector> GetElements(this Document document, string familyName, string familySymbolName)
        {
            SelectionResult<FilteredElementCollector> selectionResult = new SelectionResult<FilteredElementCollector>();
            Family family = document.GetElements<Family>(f => f.Name == familyName).FirstOrDefault();
            if (family == null)
            {
                selectionResult.Message = $"在项目中找不到名称为{familyName}的族";
            ...
            {
                selectionResult.Message = $"在族{familyName}中找不到名称为{familySymbolName}的类型";
                return selectionResult;
            }

            selectionResult.Value = document.GetElements(familySymbol);
            selectionResult.Message = "查询成功";
